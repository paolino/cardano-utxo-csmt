...
echo "Building cardano-utxo-csmt artifacts version $version"
nix build ".#$platform.tarball"

tarball=result/cardano-utxo-csmt-$version-$platform.tar.gz
echo "Tarball path: $tarball"

mktempdir=$(mktemp -d)
releaseTarball="$mktempdir/cardano-utxo-csmt-$release-$platform.tar.gz"

cp -L "$tarball" "$releaseTarball"

gh release upload "$release" "$releaseTarball"

echo "Release URL: https://github.com/paolino/cardano-utxo-csmt/releases/tag/$release"